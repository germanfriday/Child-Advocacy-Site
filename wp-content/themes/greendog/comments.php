...
<?php if (!empty($post->post_password) && $_COOKIE['wp-postpass_'.COOKIEHASH]!=$post->post_password) : ?>
	<p id="comments-locked" class="h3comm">Enter your password to view comments.</p>
<?php return; endif; ?>
...
<?php 
	...
	<h3 class="h3comm"><?php _e($numComments); ?> Comments</h3>
<!-- -->
...
<?php /* If you want to use gravatars, they go somewhere around here */ ?>
				<b><?php comment_author_link() ?></b> says <?php comment_date('jS F') ?> @ <?php comment_date('G:i') ?>
			</p>
 ...
 */ ?>
	...
	<h3 class="h3comm">Оставить комментарий</h3>
	
	<?php if (get_option('comment_registration') && !$user_ID ) : ?>
		<p id="comments-blocked">Вы должны <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=
		<?php the_permalink(); ?>">войти</a>, чтобы оставлять комментарии.</p>
	<?php else : ?>
	...
	<?php if ($user_ID) : ?>
		...
		<p class="skinnedArea">
		<label for="comment">Вы вошли как <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php">
		<?php echo $user_identity; ?></a>. 
		<a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" 
		title="Log out of this account">Выйти</a></label>
	
	<?php else : ?>
	
		<p class="skinned">
		<label for="author">Имя:<?php if ($req) _e(' (required)'); ?></label>
		<input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" />
		</p>
		
		<p class="skinned">
		<label for="email">E-mail (не будет показан на сайте):<?php if ($req) _e(' (required)'); ?></label>
	...
	<?php /* You might want to display this: 
		...
		<?php /* Buttons are easier to style than input[type=submit], 
				...
				if you like */ 
		?>
		<input type="submit" name="submit" id="sub" value="Готово" />
...
<?php else : // Comments are closed ?>
	<h3 id="comments-closed"  class="h3comm">Извините, комментарии закрыты к этой записи.</h3>
<?php endif; ?>